...
let combinedUIDs = new Set<string>();              // 所有 UID (排重)
let combinedTXIDs = new Set<string>();             // 所有 TXID (排重)
let combinedAddresses = new Set<string>();         // 所有充值地址 (排重)
...
let mergedResultList: MergedResultItem[] = [];
 ...
 * 並顯示在畫面上，尚未解析。
 */
function handleMultipleFiles(event: Event): void {
 ...
 * 顯示目前已選擇的檔案列表(檔名)
 */
function displayUploadedFileList(): void {
 ...
 */
function readXlsxFile(file: File): Promise<void> {
	return new Promise((resolve, reject) => {
		...
		reader.onload = (e: ProgressEvent<FileReader>) => {
			...
			try {
				...
				const jsonData = window.XLSX.utils.sheet_to_json(sheet) as any[];
 ...
 */
function renderAnalysisResult(): void {
	...
	if (totalCountEl) {
		totalCountEl.textContent = mergedResultList.length.toString();
	}
	...
	if (uidCountEl) {
		...
		uidCountEl.textContent = combinedUIDs.size.toString();
	}
}
 ...
 * 複製所有 UID (排除重複)
 */
function copyAllUniqueUIDs(): void {
 ...
 */
function copyAllTxidAndAddress(): void {
	// 合併 & 去重
...
// 將函數掛載到全域
(window as any).handleMultipleFiles = handleMultipleFiles;
(window as any).analyzeFiles = analyzeFiles;
(window as any).copyAllUniqueUIDs = copyAllUniqueUIDs;
(window as any).copyAllTxidAndAddress = copyAllTxidAndAddress;
